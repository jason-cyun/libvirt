...
qemuDomainObjResetAsyncJob(qemuDomainObjPrivatePtr priv)
{
    ...
    job->asyncActive = QEMU_ASYNC_JOB_NONE;
    job->asyncOwner = 0;
                        ...
                        qemuDomainJobObjPtr job)
{
    ...
    job->asyncActive = priv->job.asyncActive;
                                 ...
                                 qemuDomainObjPrivatePtr priv)
{
    ...
    if (job == QEMU_JOB_NONE &&
        priv->job.asyncActive == QEMU_ASYNC_JOB_NONE)
        return 0;
                              ...
                              bool nowait)
{
 ...
 retry:
    ...
    if (job) {
        ...
        if (job != QEMU_JOB_ASYNC) {
            ...
            // which API calls in here(virThreadJobSet is called by RPC API)
 ...
 error:
    ...
    if (priv->job.asyncActive&& priv->job.asyncStarted)
...
qemuDomainStorageAlias(const char *device, int depth)
{
 ...
 /*
  * /usr/libexec/qemu-kvm -name i-f9smezq3o1 -S -machine pc-i440fx-rhel7.0.0,accel=kvm,usb=off -cpu host -m 8192 -realtime mlock=off -smp 2,sockets=1,cores=1,threads=2 -uuid 32bf609d-50d5-4e7c-b6d8-62efd587dfe0 -smbios type=1,manufacturer=JD,product=JCloud Iaas Jvirt,version=1.0,serial=76044572-0d0a-11e7-a468-049fca013a59,uuid=32bf609d-50d5-4e7c-b6d8-62efd587dfe0,family=Virtual Machine -no-user-config -nodefaults -chardev socket,id=charmonitor,path=/var/lib/libvirt/qemu/domain-308-i-f9smezq3o1/monitor.sock,server,nowait -mon chardev=charmonitor,id=monitor,mode=control -rtc base=utc,driftfix=slew -global kvm-pit.lost_tick_policy=discard -no-hpet -no-shutdown -boot strict=on -device piix3-usb-uhci,id=usb,bus=pci.0,addr=0x1.0x2 -device virtio-serial-pci,id=virtio-serial0,bus=pci.0,addr=0x4 -drive file=/export/jvirt/jcs-agent/instances/i-f9smezq3o1/vda,format=qcow2,if=none,id=drive-virtio-disk0,cache=none,werror=report,rerror=report -device virtio-blk-pci,scsi=off,bus=pci.0,addr=0x5,drive=drive-virtio-disk0,id=virtio-disk0,bootindex=1 -netdev tap,fd=69,id=hostnet0,vhost=on,vhostfd=86 -device virtio-net-pci,netdev=hostnet0,id=net0,mac=fa:16:3e:64:96:fd,bus=pci.0,addr=0x3 -chardev file,id=charserial0,path=/export/jvirt/jcs-agent/instances/i-f9smezq3o1/console.log -device isa-serial,chardev=charserial0,id=serial0 -chardev pty,id=charserial1 -device isa-serial,chardev=charserial1,id=serial1 -chardev socket,id=charchannel0,path=/var/lib/libvirt/qemu/org.qemu.guest_agent.0.i-f9smezq3o1.sock,server,nowait -device virtserialport,bus=virtio-serial0.0,nr=1,chardev=charchannel0,id=channel0,name=org.qemu.guest_agent.0 -device usb-tablet,id=input0,bus=usb.0,port=1 -vnc 0.0.0.0:35 -k en-us -vga std -global VGA.vgamem_mb=16 -device virtio-balloon-pci,id=balloon0,bus=pci.0,addr=0x6 -msg timestamp=on
 */
int
