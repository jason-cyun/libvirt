                                ...
                                unsigned int flags)
{
    ...
    // the first rpc all runs at dst called by source
    virCheckFlagsGoto(QEMU_MIGRATION_FLAGS, cleanup);
    if (virTypedParamsValidate(params, nparams, QEMU_MIGRATION_PARAMETERS) < 0)
        goto cleanup;

    // extract parameters to separate variable
    if (virTypedParamsGetString(params, nparams,
                                VIR_MIGRATE_PARAM_DEST_XML,
                                &dom_xml) < 0 ||
        virTypedParamsGetString(params, nparams,
    ...
    // get migration parameters(used by qemu), later on we will set them by QMP command
                               ...
                               int cancelled)
{
    ...
    virCheckFlags(QEMU_MIGRATION_FLAGS, NULL);
    // each migration rpc, validate migration parameters
    if (virTypedParamsValidate(params, nparams, QEMU_MIGRATION_PARAMETERS) < 0)
        return NULL;

    // it's either set by user or from vm->def->name
    if (virTypedParamsGetString(params, nparams,
                                VIR_MIGRATE_PARAM_DEST_NAME,
                                &dname) < 0)
        return NULL;
    ...
    // find the vm object we create at Prepare phase
    vm = virDomainObjListFindByName(driver->domains, dname);
    if (!vm) {
        virReportError(VIR_ERR_NO_DOMAIN,
                       _("no domain with matching name '%s'"), dname);
    ...
    // cookiein is cookie generated at source when PERFORM3
    // cookieout should generated by me here FINISH3
    return qemuMigrationDstFinish(driver, dconn, vm,
                                  cookiein, cookieinlen,
                                  cookieout, cookieoutlen,
                                  flags, cancelled, true);
